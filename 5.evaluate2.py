...
def start_calculate():
    ...
    data = pd.read_csv(f"../tmp/parameters/parameters_woR3.csv")
    data["BER"] = data["BER"].apply(str_to_list)
    data["BER"] = data["BER"].apply(lambda x: x[0])
    data = data[data["BER"] < 0.0003]

    try:
        df = pd.read_csv(f"../tmp/parameters/evaluate_woR3.csv")
        print("read the data")
    except:
        print("create the csv file")
        df = pd.DataFrame(columns=["Vb1", "Vb2", "R1", "R2", "BER", "tse", "BER_list"])
    ...
    for line in range(len(df), len(data)):
        ...
        R3 = 1e2000
        time1 = time.time()
        BER_list = calculate_BER_trace(R1, Vb1, Vb2, R2, R3, nSymbolVector=2, SNR=25, func=1, kinds=50000, noise=False,
                                       std=0, thermal1=0, thermal2=0, shot=0, on=1)
        time2 = time.time()
        print(time2-time1)
        column["BER_list"] = BER_list
        df.loc[line] = column
        df.to_csv(f"../tmp/parameters/evaluate_woR3.csv", index=False)
        print(f"第{line}行保存成功,BER_list为{BER_list}")
